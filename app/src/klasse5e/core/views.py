...
def service_worker(request):
    script = """const CACHE='klassid-shell-v4';self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(['/offline/','/static/css/dist/styles.css?v=tailwind-v1']))));self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE).map(k=>caches.delete(k))))));self.addEventListener('fetch',e=>{if(e.request.mode==='navigate')e.respondWith(fetch(e.request).catch(()=>caches.match('/offline/')))});self.addEventListener('push',e=>{let d={title:'KlassID',body:'Neue geschützte Information',url:'/'};try{d={...d,...e.data.json()}}catch(_){}e.waitUntil(self.registration.showNotification(d.title,{body:d.body,tag:d.tag||d.message_id,data:{url:d.url},icon:d.icon||'/static/icons/klassid.svg'}))});self.addEventListener('notificationclick',e=>{e.notification.close();e.waitUntil(clients.openWindow(e.notification.data?.url||'/'))});"""
