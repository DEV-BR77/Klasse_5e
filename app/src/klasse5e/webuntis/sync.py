...
from .adapter import WebUntisAdapter, classify_error, CAPABILITIES
...
def begin_run(connection, *, trigger, idempotency_key=None, minimum_minutes=None):
    ...
    minimum = minimum_minutes or (10 if trigger == SyncRun.Trigger.MANUAL else SyncSchedule.current().min_interval_minutes)
    if latest and latest.status == SyncRun.Status.RUNNING and latest.started_at > timezone.now() - timedelta(minutes=30):
        return latest, False
...
def execute_run(run):
    categories = list(run.connection.features.filter(enabled=True).values_list("key", flat=True))
    try:
        adapter = WebUntisAdapter(server=run.connection.server, school=run.connection.school, username=decrypt(run.connection.username_encrypted), password=decrypt(run.connection.password_encrypted))
        with adapter.client:
