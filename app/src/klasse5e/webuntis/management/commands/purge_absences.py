...
from klasse5e.webuntis.models import AbsenceDraft, AbsenceSubmission, WebUntisAbsence
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        AbsenceSubmission.objects.filter(created_at__lt=timezone.now() - timedelta(days=30)).delete()
        ...
        for item in WebUntisAbsence.objects.select_related("connection__user"):
            if not absence_data_allowed(item.connection.student):
                item.delete()
        for item in AbsenceSubmission.objects.select_related("user"):
            if item.student_id not in {c.student.pk for c in child_contexts(item.user)}:
                item.delete()
